...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($fullname) || empty($email) || empty($password) || empty($confirm)) {
    ...
    if ($check->num_rows > 0) {
      ...
      $stmt = $conn->prepare("INSERT INTO accounts (fullname, email, password) VALUES (?, ?, ?)");
      $stmt->bind_param("sss", $fullname, $email, $hashedPassword);

      if ($stmt->execute()) {
        // Redirect to login
        header("Location: loginaccounts.php");
        exit();
      } else {
...
<body>
  ...
  <div class="form-container">
    ...
    <a class="link" href="loginaccounts.php">Already have an account? Login</a>
  </div>
